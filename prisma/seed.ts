...
async function main() {
	...
	await prisma.muscle_groups.upsert({
		where: { id: 1 },
		create: { name: "Abdominals", id: 1 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 2 },
		create: { name: "Arms", id: 2 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 3 },
		create: { name: "Back", id: 3 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 4 },
		create: { name: "Calves", id: 4 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 5 },
		create: { name: "Chest", id: 5 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 6 },
		create: { name: "Compound", id: 6 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 7 },
		create: { name: "Glutes", id: 7 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 8 },
		create: { name: "Legs", id: 8 },
		update: {},
	});
	await prisma.muscle_groups.upsert({
		where: { id: 9 },
		create: { name: "Shoulders", id: 9 },
		update: {},
	});
}
main()
