...
const formatExerciseData = (exercise: exercises & { muscle_groups?: any }) => {
  return {
    ...exercise,
...
export const exerciseService = {
  ...
  updateExercise: async (id: number, input: UpdateExerciseInput) => {
    ...
    if (input.defaultIncrementKg !== undefined) updateData.defaultIncrementKg = toDecimal(input.defaultIncrementKg);
    if (input.minWeight !== undefined) updateData.minWeight = toDecimal(input.minWeight);
    if (input.maxWeight !== undefined) updateData.maxWeight = toDecimal(input.maxWeight);
